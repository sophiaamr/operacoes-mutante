...
test('7. deve retornar o resto da divisão', () => { expect(restoDivisao(10, 3)).toBe(1); });
  ...
  expect(() => fatorial(-1)).toThrow(/Fatorial.*negativos/i); });
  test('8.1 fatorial(1)=1', () => expect(fatorial(1)).toBe(1));
  test('8.2 fatorial(0)=1', () => expect(fatorial(0)).toBe(1));
  test('8.3 fatorial(0)=1', () => expect(() => fatorial(-1)).toThrow('Fatorial não é definido para números negativos.'));
  test('9. deve calcular a média de um array com múltiplos elementos', () => { expect(mediaArray([10, 20, 30])).toBe(20); });
  test('9.1 deve calcular a média de um array com múltiplos elementos', () => { expect(mediaArray([])).toBe(0); });
  ...
  test('11.1 deve encontrar o valor máximo em um array', () => { expect(() => maximoArray([])).toThrow('Array vazio не possui valor máximo.'); });

  test('12. deve encontrar o valor mínimo em um array', () => { expect(minimoArray([10, 2, 100])).toBe(2); });
    test('12. minimoArray', () => expect(minimoArray([5, -2, 9])).toBe(-2));
  test('12.1 minimoArray([]) lança mensagem', () => {
  ...
  test('15.2 isPar retorna false para ímpar', () => {
  expect(isPar(3)).toBe(false);
});
  test('16. deve retornar true para um número ímpar', () => { expect(isImpar(7)).toBe(true); });
    test('16.2 deve retornar false para um número par', () => { expect(isImpar(2)).toBe(false); });
  ...
  test('33.2 deve verificar que um número é primo', () => { expect(isPrimo(6)).toBe(false); });
  test('33.2 deve verificar que um número é primo', () => { expect(isPrimo(1)).toBe(false); });
  test('33.2 deve verificar que um número é primo', () => { expect(isPrimo(0)).toBe(false); });
  ...
  test('36.2 clamp retorna max quando valor > max', () => {
  expect(clamp(5, 0, 10)).toBe(5);
  expect(clamp(11, -3, 10)).toBe(10);
  expect(clamp(-5, -3, 10)).toBe(-3);
  expect(clamp(4, -3, 10)).toBe(4);
  ...
  test('37.2 deve verificar se um número é divisível por outro', () => { expect(isDivisivel(11, 2)).toBe(false); });
  ...
  test('40. deve calcular o inverso de um número', () => { expect(() => inverso(0)).toThrow('Não é possível inverter o número zero.'); });
  ...
  test('44. deve verificar se um número é maior que outro', () => { expect(isMaiorQue(10, 10)).toBe(false); });


  test('45. deve verificar se um número é menor que outro', () => { expect(isMenorQue(5, 10)).toBe(true); });
  test('45. deve verificar se um número é menor que outro', () => { expect(isMenorQue(10, 10)).toBe(false); });


  test('46. deve verificar se dois números são iguais', () => { expect(isEqual(7, 7)).toBe(true); });
  test('46. deve verificar se dois números são iguais', () => { expect(isEqual(7, 8)).toBe(false); });
